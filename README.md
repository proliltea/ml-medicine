# ml-medicine
🚀 Использование моделей (Inference)
Для работы моделей необходим Python 3.8+ и библиотеки: pandas, joblib, catboost, xgboost.
1. 🧬 Pancreatic Cancer Model (pancreatic_cancer_model.joblib)
Модель для предсказания риска рака поджелудочной железы (Binary Classification).
Алгоритм: CatBoost.
Входные данные (Raw Input):
Признак	Описание
age	Возраст (лет)
sex	Пол ('M' или 'F')
plasma_CA19_9	Анализ крови (ед/мл). Можно NaN.
creatinine	Креатинин в моче (мг/дл)
LYVE1, REG1B, TFF1	Биомаркеры мочи (нг/мл)
⚙️ Препроцессинг (Обязательно!)
Перед подачей в model.predict() необходимо создать синтетические признаки:
code
Python
# df - DataFrame с сырыми данными
# 1. Маппинг пола
df['sex'] = df['sex'].map({'M': 0, 'F': 1})

# 2. Нормализация на креатинин
df['LYVE1_ratio'] = df['LYVE1'] / df['creatinine']
df['REG1B_ratio'] = df['REG1B'] / df['creatinine']
df['TFF1_ratio']  = df['TFF1']  / df['creatinine']

# 3. Взаимодействие признаков (Interactions)
df['LYVE1_x_REG1B'] = df['LYVE1_ratio'] * df['REG1B_ratio']
df['REG1B_x_TFF1']  = df['REG1B_ratio'] * df['TFF1_ratio']

# Порядок колонок важен для CatBoost!
cols = ['age', 'sex', 'plasma_CA19_9', 'creatinine', 'LYVE1', 'REG1B', 'TFF1', 
        'LYVE1_ratio', 'REG1B_ratio', 'TFF1_ratio', 'LYVE1_x_REG1B', 'REG1B_x_TFF1']
prediction = model.predict_proba(df[cols])[:, 1]
2. ❤️ Cardiovascular Disease Model (cardio_model.joblib)
Модель оценки риска сердечно-сосудистых заболеваний.
Алгоритм: XGBoost.
Входные данные (Raw Input):
Признак	Описание	Важно
age	Возраст	В днях! (Лет * 365)
gender	Пол	1 = Ж, 2 = М
height / weight	Рост (см) / Вес (кг)	
ap_hi / ap_lo	Давление (верх/низ)	
cholesterol	Холестерин	1 (норм), 2 (выш), 3 (выс)
gluc	Глюкоза	1 (норм), 2 (выш), 3 (выс)
smoke / alco / active	Привычки	0 (нет) / 1 (да)
⚙️ Препроцессинг (Обязательно!)
Необходимо рассчитать пульсовое давление.
code
Python
# df - DataFrame с сырыми данными
# 1. Создание признака ap_diff

df['ap_diff'] = df['ap_hi'] - df['ap_lo']

# 2. Удаление лишнего (если есть id)

if 'id' in df.columns: df = df.drop(columns=['id'])

# 3. Предсказание (Рекомендуемый порог 0.45 для Recall)

probs = model.predict_proba(df)[:, 1]

result = (probs > 0.45).astype(int)
